...
public class Simulation extends Thread {
  ...
  private void startSimulation() {
    while (running) {
      ...
      //  zum angucken vom feld
      for(int i = 0; i < field.length; i++) {
        for(int j = 0; j < field[0].length; j++) {
        int neighbourCount = getNeighbourCount(i,j); // hab ich eine konkrete anzahl and nachbarn?
        if (field[i][j] == 0 && neighbourCount == 3) {
          newField[i][j] = 1;
        } else {
          if (field[i][j] == 1 && (neighbourCount == 2 || neighbourCount == 3)) {
            newField[i][j] = 1;
          } else {
            newField[i][j] = 0;
          }
      ...
      // if new array is equal to previous array, stopSimulation()


      /*
       * ALMOST DONE: Implement the main logic used for running the simulation. Make sure to
  ...
  private int getNeighbourCount(int x, int y) {
    ...
    for (int zx = -1; zx <= 1; zx++) {
      for (int zy = -1; zy <= 1; zy++) {
        ...
        if (nx >= 0 && ny >= 0 && nx < field.length && ny < field[0].length && coordsInBounds(nx, ny)) {
          if (field[nx][ny] == 1) {
            neighbourCount++;
          }
        }
    ...
    /*
     * DONE: Implement logic that counts the neighbours of a given cell. Neighbours
  ...
  private boolean coordsInBounds(int x, int y) {
    ...
    if (x <= 0 && y <= 0 && x > field.length && y > field.length) {
      // if x and y smaller than 0 or bigger than the max size then return false
  ...
  // return 0 <= x && x < field.length && 0 <= y && y < field.length;
    ...
    /*
     * DONE: Implement logic that checks if the given coordinates are in bounds,
